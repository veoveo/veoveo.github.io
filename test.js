...
// hello1
document.getElementsByClassName("cdmYfE")[0].remove();
let claim_status = 0;
let coin = 500;
...
let time_claim = 600;
let token_list = [
"EP4DLUu2s2+TeO0LFaVxY8wmIO1H6vHONaI0WWaewcvLREWaYmbQfVKp+8ogz3sh",
"EB/xdNd3aM8aZJERsGBzO1gRCXriA/CHWDIkCXGC2Vj6ihoINjGl7poEGa5K9bZZU7DcnyZVlCsae4Rl1yqS",
"Eb5wdsjwgTHLFL852qswGEds8pfZCv0xf80EVBJeIp5EyugaxtL9PYyxsBi9/kpbb7FT+3CQFK18oePw2XqIjUxsBCmM",
...
"jTmPDrIFaFktO001hCohAWYg//56ny7SSJar4eoXveH/2ZjXQHsj0qSaXOQfbCFZ",
"GpXIWCDqZvmXFgALldx/6CKRYQqxqHEXGLCty2Pv0MGCe8FiU35GaFacpNcqE6Io",
"9M/8pbgZxa+AbVO8WKyl8/IGSnY/x2aUzm/T8lqoQARbT8eGflx8moJE9WUNR2zc"
];
function toggleCheckbox(element)
 {
     if (document.getElementById("xurot").checked) {
         Checkbox = true;
         lock_coin_xurot();
     } else {
         Checkbox = false;
...
function claimed_vc() {
    document.getElementsByClassName("kohnpI")[0].style.display = "block";
document.getElementsByClassName("cdSlkL")[0].textContent = "đã lụm voucher";
setTimeout(function(){
      document.getElementsByClassName("kohnpI")[0].style.display = "none";
    }, 1500)
}
 
function check_login() {
    var xhr = new XMLHttpRequest();
    var url = `https://live.shopee.vn/api/v1/session/${se}/coin/user_config?uid=523499622&utm_campaign=-&utm_content=test----&utm_medium=affiliates&utm_source=an_17333510032`;
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          if (responseData.data.required_watch_time==0) {
                window.location="https://shopee.vn/buyer/login?next=https://live.shopee.vn/share";
          } else {
                id_coin = responseData.data.coin_id;
                time_claim = responseData.data.required_watch_time;
          }
        } else {
...
<button onclick="refresh_coin()"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-repeat" viewBox="0 0 16 16">
...
</svg></button><input type="checkbox" id="xurot" onchange="toggleCheckbox()"><label>xu rớt</label>`;
document.getElementsByClassName("app-wrapper")[0].style["justify-content"] = "right";
...
function claim_voucher(vc) {
    var xhr = new XMLHttpRequest();
    var url = `https://live.shopee.vn/api/v1/voucher/claim`;
    var data = JSON.stringify({
        "voucher_code": vc["voucher_code"],
        "session_id": parseInt(se),
        "promotion_id": vc["promotion_id"],
...
function lock_coin(coin_id, ss_id, li_token1) {
    let li_token = [...li_token1];
    var now_token = li_token.pop();
    document.cookie = "shopee_token="+now_token+"; path=/";
    setTimeout(function() {
                  if (li_token.length != 0) {
                      lock_coin(coin_id, ss_id, li_token);
                  }
                }, 10);
    var xhr = new XMLHttpRequest();
      ...
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          var responseData = JSON.parse(xhr.responseText);
            ...
            if (now_token=="EP4DLUu2s2+TeO0LFaVxY8wmIO1H6vHONaI0WWaewcvLREWaYmbQfVKp+8ogz3sh") {
...
function lock_coin_xurot() {
    var xhr = new XMLHttpRequest();
    var url = `https://live.shopee.vn/api/v1/session/${se}/coin/lock`;
    var data = JSON.stringify({"uid": 1756170, "coin_id": id_coin});
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          if (responseData.err_code == 0 && claim_status == 0) {
            var tsl = responseData.data.require_wait_time;
            check_coin(id_coin, se, tsl);
          ...
          if (responseData.data.has_another_device) {
              ...
              remove_log();
                ...
                if (responseData.data.coins_per_claim>=coin && responseData.data.remain_locks>1 && claim_status == 0) {
                        lock_coin(responseData.data.coin_id, se, token_list);
                  }
          }
        } else {
          console.log("(can_claim) Yêu cầu GET không thành công. Mã trạng thái:", xhr.status);
...
function check_fail_coin(coin_id, ss_id) {
    var xhr = new XMLHttpRequest();
    var url = `https://live.shopee.vn/api/v1/session/${ss_id}/coin/user_config?uid=523499622&utm_campaign=-&utm_content=test----&utm_medium=affiliates&utm_source=an_17333510032`;
    xhr.open("GET", url, true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
      ...
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          if (responseData.data.coins_per_claim>=coin) {
                ...
                setTimeout(function() {
                  claim_status = 0;
                  can_claim(coin_id, ss_id, token_list);
                }, (tsl+3)*1000); // chờ n giây
...
function can_claim(coin_id, ss_id, li_token1) {
    let li_token = [...li_token1];
    document.cookie = "shopee_token="+li_token.pop()+"; path=/";
    var xhr = new XMLHttpRequest();
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          //claim(coin_id, ss_id)
          if (li_token.length!=0) {
            can_claim(coin_id, ss_id, li_token);
        } else {
            claim(coin_id, ss_id, token_list);
        }
        } else {
...
function claim(coin_id, ss_id, li_token1) {
    let li_token = [...li_token1];
    document.cookie = "shopee_token="+li_token.pop()+"; path=/";
    var xhr = new XMLHttpRequest();
...
(window.webpackJsonp_N_E = window.webpackJsonp_N_E || []).push([[41], {
    ...
    Pc2E: function(e, t, r) {
        ...
        Object(S.a)(i, c.SESSION_SHOW_VOUCHER, O("ShowVoucherMsg")),
        ...
        i), A = function(e, t) {
            if (e==3011 && claim_status==0) {
                // thông báo có xu
                id_coin = w[1004](t)[1].duration;
                lock_coin(id_coin, se, token_list);
                //alert(w[1004](t)[1].duration)
            }
            if (e==1006) {
                // thông báo có xu
                try {
                    claim_voucher(JSON.parse(w[e](t)[1].voucher))
                } catch (error) {
            ...
            j || ((j = new o.a({
                ...
                generateUrl: function() {
                    return function(e, t, r) {
                        var i = new URL("wss://".concat(s.F, "/im/v1/comet"));
                ...
                if (i.type === l.PUSH_REQ) {
                    ...
                    try {
                        ...
                        E && console.log("%cReceive Message: ".concat(p), "background: green", c),
                        Object(T.j)({
                            sessionId: e,
                            msgType: s,
...
//# 41.97ffd1eb735dfffde67a.js.map
